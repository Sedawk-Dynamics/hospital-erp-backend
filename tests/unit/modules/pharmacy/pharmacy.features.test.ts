...
describe('Pharmacy — feature coverage (G1–G17 + credit gate + GRN gaps)', () => {
  ...
  describe('reports (G15)', () => {
    ...
    it('vendor-wise aggregates paid value + medicines per supplier', async () => {
      (prisma.drugBatch.findMany as any).mockResolvedValue([
        { drugId: 'd1', supplierId: 's1', quantityReceived: 100, quantityInStock: 80, purchasePrice: 10, purchaseDiscountPercent: 0, supplier: { id: 's1', name: 'Acme' } },
        { drugId: 'd2', supplierId: 's1', quantityReceived: 50, quantityInStock: 50, purchasePrice: 20, purchaseDiscountPercent: 10, supplier: { id: 's1', name: 'Acme' } },
      ]);
      const r = await getVendorWiseReport(TENANT_ID);
      expect(r.items[0]).toMatchObject({ supplierId: 's1', supplierName: 'Acme', medicineCount: 2, batchCount: 2 });
